#!/bin/bash
#SBATCH --job-name=ndl_crp_fbr_invw_ro
...
#SBATCH --output=/home/nhoffma1/scratch.fuge-prj/needle_mgn/experiments/cropped_fiber_iso_invw_rollout/slurm-%j.out
#SBATCH --error=/home/nhoffma1/scratch.fuge-prj/needle_mgn/experiments/cropped_fiber_iso_invw_rollout/slurm-%j.err

# Multi-step rollout variant of cropped_fiber_iso_invw.
#
# Curriculum: L = (1 - w) * L_1step + w * L_5step
#   - First multistep_warmup_epochs (5): w = 0  (pure 1-step training).
#   - Remaining epochs: w ramps linearly from 0 to multistep_w_max (0.7).
#
# Per rollout step inside training:
#   - Predicted Δ is denormalised, raw needle position is updated.
#   - World (contact) edges are rebuilt from the new positions via the
#     same cKDTree / world_edge_radius algorithm used in infer.py.  Hex
#     mesh edges stay fixed (topology-invariant).
#   - edge_attr (rel_pos, edge_len) is recomputed against the live pos
#     tensor so gradients flow through the geometry.
#
...
EXP=/home/nhoffma1/scratch.fuge-prj/needle_mgn/experiments/cropped_fiber_iso_invw_rollout
