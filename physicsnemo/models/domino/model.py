...
from physicsnemo.nn.module.fourier_feature_transform import FourierFeatureTransform
...
class DoMINO(Module):
    ...
    def __init__(
        ...
        node_state_dim_vol: int = 0,
        node_state_dim_surf: int = 0,
        use_fourier_features_state: bool = False,
        n_fourier_features_state: int = 64,
        fourier_scale_state: float = 1.0,
    ):
        ...
        self.node_state_dim_vol = node_state_dim_vol
        self.node_state_dim_surf = node_state_dim_surf

        # Optional Fourier feature transform on injected state features
        if use_fourier_features_state and node_state_dim_vol > 0:
            self.state_fourier_vol = FourierFeatureTransform(
                node_state_dim_vol, n_fourier_features_state, fourier_scale_state
            )
            effective_state_dim_vol = self.state_fourier_vol.out_features
        else:
            self.state_fourier_vol = None
            effective_state_dim_vol = node_state_dim_vol

        if use_fourier_features_state and node_state_dim_surf > 0:
            self.state_fourier_surf = FourierFeatureTransform(
                node_state_dim_surf, n_fourier_features_state, fourier_scale_state
            )
            effective_state_dim_surf = self.state_fourier_surf.out_features
        else:
            self.state_fourier_surf = None
            effective_state_dim_surf = node_state_dim_surf
        ...
        if self.output_features_surf is not None:
            inp_pos_surf = 3 + effective_state_dim_surf
    ...
    def forward(
        ...
        if self.output_features_vol is not None:
            ...
            # Inject per-node state features (DCEL extension)
            if self.node_state_dim_vol > 0 and "state_vol" in data_dict:
                state_vol = data_dict["state_vol"]
                if self.state_fourier_vol is not None:
                    state_vol = self.state_fourier_vol(state_vol)
                encoding_node_vol = torch.cat([encoding_node_vol, state_vol], dim=-1)
        ...
        if self.output_features_surf is not None:
            ...
            # Inject per-node state features (DCEL extension)
            if self.node_state_dim_surf > 0 and "state_surf" in data_dict:
                state_surf = data_dict["state_surf"]
                if self.state_fourier_surf is not None:
                    state_surf = self.state_fourier_surf(state_surf)
                encoding_node_surf = torch.cat([encoding_node_surf, state_surf], dim=-1)
