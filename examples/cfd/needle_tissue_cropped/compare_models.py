...
class RolloutEngine:
    def __init__(self, model, dataset: NeedleTissueDataset, cfg, device: torch.device,
        ...
        # Rigid-body bias correction — subtracts cached per-step Δt + Δω×r
        # from predicted normalised Δu on needle nodes before applying the
        # state update.  Generated by compute_rigid_correction.py on the
        # training split.
        self.apply_rigid_correction = bool(cfg.get("apply_rigid_correction", False))
        self.rigid_correction_per_step = bool(cfg.get("rigid_correction_per_step", True))
        self._rigid_correction = None
        if self.apply_rigid_correction:
            self._load_rigid_correction(cfg)
    ...
    def _load_rigid_correction(self, cfg):
        path = cfg.get("rigid_correction_path", None)
        if path is None:
            # Fall back to <stats_dir>/rigid_correction.pt — same convention
            # as compute_rigid_correction.py.
            stats_dir = self.dataset.stats_path if hasattr(self.dataset, "stats_path") else None
            if stats_dir is None:
                raise FileNotFoundError(
                    "apply_rigid_correction=true but no rigid_correction_path "
                    "in cfg and the dataset doesn't expose stats_path."
                )
            path = os.path.join(stats_dir, "rigid_correction.pt")
        if not os.path.exists(path):
            raise FileNotFoundError(
                f"apply_rigid_correction=true but no cache at {path}.  "
                f"Run compute_rigid_correction.py first."
            )
        rc = torch.load(path, weights_only=False)
        self._rigid_correction = {
            "delta_t_norm": rc["delta_t_norm"].to(self.device),
            "delta_omega_norm": rc["delta_omega_norm"].to(self.device),
            "delta_t_norm_mean": rc["delta_t_norm_mean"].to(self.device),
            "delta_omega_norm_mean": rc["delta_omega_norm_mean"].to(self.device),
            "n_steps": int(rc["n_steps"]),
            "n_samples": int(rc["n_samples"]),
        }
        print(
            f"[RolloutEngine] apply_rigid_correction loaded from {path} "
            f"(n_samples={rc['n_samples']}, K={rc['n_steps']}, "
            f"per_step={self.rigid_correction_per_step})",
            flush=True,
        )

    def _apply_rigid_correction(self, pred: torch.Tensor, graph: Data,
                                step_k: Optional[int]) -> torch.Tensor:
        """Return a corrected copy of pred with Δt_k + Δω_k × r subtracted
        from the u-channel on needle nodes."""
        if self._rigid_correction is None or "u" not in self.tgt_offsets:
            return pred
        ...
        rc = self._rigid_correction
        if self.rigid_correction_per_step and step_k is not None:
            k_idx = min(int(step_k), rc["n_steps"] - 1)
            dt = rc["delta_t_norm"][k_idx]
            dw = rc["delta_omega_norm"][k_idx]
        else:
            dt = rc["delta_t_norm_mean"]
            dw = rc["delta_omega_norm_mean"]

        pos_needle = graph.pos[needle].float()
        centroid = pos_needle.mean(dim=0)
        r = pos_needle - centroid
        rigid = dt.view(1, 3) + torch.cross(
            dw.view(1, 3).expand_as(r), r, dim=-1
        )                                                  # (n_needle, 3)
        u_lo, u_hi = self.tgt_offsets["u"]
        out = pred.clone()
        out[needle, u_lo:u_hi] = out[needle, u_lo:u_hi] - rigid
        return out
    ...
    def step(self, graph: Data, pred: torch.Tensor,
             step_k: Optional[int] = None) -> Data:
        pred = pred.float()
        if self._rigid_correction is not None:
            pred = self._apply_rigid_correction(pred, graph, step_k)
        x = graph.x.float()
        new_x = x.clone()
    ...
    def step_and_clamp(self, graph: Data, pred: torch.Tensor,
        ...
        t_local + k + 1.  When ctx is None the override no-ops.  step_k
        is forwarded so the rigid-body bias correction can index its cache
        per rollout step."""
        self.step(graph, pred, step_k=k)
        if ctx is not None:
            r_idx, t_local = ctx
