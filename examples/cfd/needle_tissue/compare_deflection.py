...
Usage (from examples/cfd/needle_tissue/):
    python compare_deflection.py [--infer_dir ./inference_output]
                                 [--data_dir /path/to/RUN-2]
                                 [--beam_spacing_mm 1.0]
                                 [--out_dir ./deflection_plots]
"""
...
def _sorted_vtu_files(data_dir: str):
    pattern = os.path.join(data_dir, "output_*.vtu")
    files = sorted(glob.glob(pattern), key=lambda p: int(re.search(r"(\d+)", os.path.basename(p)).group(1)))
    if not files:
        raise FileNotFoundError(f"No output_*.vtu files found in {data_dir}")
    return files


def _get_needle_indices(data_dir: str, beam_spacing_mm: float) -> np.ndarray:
    ...
    if beam_spacing_mm > 0.0:
        ...
        cache = torch.load(beam_path, weights_only=False)
        return cache["needle_node_indices"].numpy()
    else:
        # Fall back to reading cell types from frame 0 VTU
        vtu_files = _sorted_vtu_files(data_dir)
        mesh = pv.read(vtu_files[0])
        cell_types = mesh.celltypes
        conn = mesh.cells  # flat packed array
        # HEX cells (type 12), element_type stored in cell array "element_type"
        # element_type=0 → needle cells
        elem_types = mesh.cell_data.get("element_type", np.zeros(mesh.n_cells, dtype=np.int32))
        needle_node_set = set()
        offset = 0
        for i, ct in enumerate(cell_types):
            n_pts = conn[offset]
            pts = conn[offset + 1 : offset + 1 + n_pts]
            if ct == 12 and elem_types[i] == 0:
                needle_node_set.update(pts.tolist())
            offset += n_pts + 1
        return np.array(sorted(needle_node_set), dtype=np.int64)
...
def main():
    ...
    parser.add_argument("--beam_spacing_mm", type=float, default=1.0,
                        help="Beam spacing used during training (for cache lookup)")
    ...
    needle_idx = _get_needle_indices(args.data_dir, args.beam_spacing_mm)
    print(f"Found {len(needle_idx)} needle nodes in original mesh.")
    ...
    for step, fpath in enumerate(pred_files):
        ...
        # Predicted positions: mesh.points now contains model-predicted geometry
